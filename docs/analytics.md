...
| Search Console | Domain property `sc-domain:inyourfacecomedy.ch`. The same service account is a Restricted user on it (added 2026-09-18) and the Search Console API is enabled on `inyourface-ga-mcp`; `script/gsc-report.ts` runs the weekly learning loop into `seo/` (`search-console.md`, `seo/README.md`) |
